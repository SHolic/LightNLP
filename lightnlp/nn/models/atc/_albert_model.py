...
class AlbertClassificationModel(nn.Module):
    def __init__(self,
        ...
        self.albert = AlbertLayer(pre_trained_model_path=pre_trained_model_path, finetune=finetune)
        self.linear1 = LinearLayer(input_dim=self.albert.config.hidden_size,
    ...
    def forward(self, inputs=None, labels=None, mask=None, **kwargs):
        if not self.finetune:
            self.albert.eval()
            with torch.no_grad():
                _, x1 = self.albert(input_ids=inputs, attention_mask=mask)
        else:
            _, x1 = self.albert(input_ids=inputs, attention_mask=mask)
