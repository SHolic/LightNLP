...
class NERTrainer(BaseTrainerMixin):
    ...
    def train(self, model, train_data, dev_data, optimizer, scheduler,
        ...
        for epoch in range(self.epoch_num):
            ...
            for i, train in enumerate(train_data):
                ...
                optimizer.zero_grad()
                logits, loss = model(input_ids=train_input_ids, labels=train_labels, mask=train_mask)
            ...
            for i, test in enumerate(dev_data):
                ...
                with torch.no_grad():
                    pred_label, loss = model(input_ids=test_input_ids, labels=test_label, mask=test_mask)
    ...
    def predict(self, model, test_data, summary_writer, verbose=1, label2idx=None):
        ...
        for i, test in ctqdm(iterable=enumerate(test_data), desc="Batch test",
            ...
            with torch.no_grad():
                pred_label, loss = model(input_ids=test_input_ids, test_mask=test_mask)
